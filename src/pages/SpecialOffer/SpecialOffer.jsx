import React from 'react'
import './SpecialOffer.scss'

import Corn from '../../assets/кукуруза_1.png'
import Paper from '../../assets/products/paper.png'
import Honey from '../../assets/products/honey.png'


function Special() {
  return (
    <section className='special__offer'>
<div className='container'>
<h1 className="special__offer-title">Специальные предложения</h1>

<ul className="special__offer-list">
  <li className="special__offer-items">Акции</li>
  <li className="special__offer-items">Новинки</li>
  <li className="special__offer-items">Все</li>
</ul>

{/* -------------------------------------------------------------- */}

<div className="special__offer-products-box">
    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Corn} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Кукуруза “Kidzo”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>
    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Honey} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Мёд “ASAL”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>

    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Paper} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Туалетная бумага “Нежность”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>
{/* ------------------------------------------------------- */}

    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Corn} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Кукуруза “Kidzo”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>
    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Honey} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Мёд “ASAL”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>

    
<div className="special__offer-products">
    <div className="special__offer-img-box">
    <h3 className="special__offer-product-title">АКЦИОННЫЙ
ТОВАР</h3>
    <p className="special__offer-discount"><span>-30%</span></p>
    <div className="special__offer-price-box">
    <img src={Paper} alt="" className="special__offer-img" />
<div>
        <h3 className="special__offer-description">“ASAL”  Мёд</h3>
        <p className="special__offer-old-price">3000</p>
        <h2 className="special__offer-price">17.<span>500</span></h2>
        <h2 className="special__offer-currency">UZS</h2>
        </div>
      </div>
    </div>

<div className="special__offer-bottom-box">
  <h3 className="special__offer-bottom-title">Туалетная бумага “Нежность”</h3>
  <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
  <button className="special__offer-bottom-btn">Подробнее...</button>
    </div>
  </div>

    
    <div className="special__offer-products">
        <div className="special__offer-img-box">
        <h3 className="special__offer-product-title">АКЦИОННЫЙ
    ТОВАР</h3>
        <p className="special__offer-discount"><span>-30%</span></p>
        <div className="special__offer-price-box">
        <img src={Corn} alt="" className="special__offer-img" />
    <div>
            <h3 className="special__offer-description">“ASAL”  Мёд</h3>
            <p className="special__offer-old-price">3000</p>
            <h2 className="special__offer-price">17.<span>500</span></h2>
            <h2 className="special__offer-currency">UZS</h2>
            </div>
          </div>
        </div>
    
    <div className="special__offer-bottom-box">
      <h3 className="special__offer-bottom-title">Кукуруза “Kidzo”</h3>
      <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
      <button className="special__offer-bottom-btn">Подробнее...</button>
        </div>
      </div>
        
    <div className="special__offer-products">
        <div className="special__offer-img-box">
        <h3 className="special__offer-product-title">АКЦИОННЫЙ
    ТОВАР</h3>
        <p className="special__offer-discount"><span>-30%</span></p>
        <div className="special__offer-price-box">
        <img src={Honey} alt="" className="special__offer-img" />
    <div>
            <h3 className="special__offer-description">“ASAL”  Мёд</h3>
            <p className="special__offer-old-price">3000</p>
            <h2 className="special__offer-price">17.<span>500</span></h2>
            <h2 className="special__offer-currency">UZS</h2>
            </div>
          </div>
        </div>
    
    <div className="special__offer-bottom-box">
      <h3 className="special__offer-bottom-title">Мёд “ASAL”</h3>
      <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
      <button className="special__offer-bottom-btn">Подробнее...</button>
        </div>
      </div>
    
        
    <div className="special__offer-products">
        <div className="special__offer-img-box">
        <h3 className="special__offer-product-title">АКЦИОННЫЙ
    ТОВАР</h3>
        <p className="special__offer-discount"><span>-30%</span></p>
        <div className="special__offer-price-box">
        <img src={Paper} alt="" className="special__offer-img" />
    <div>
            <h3 className="special__offer-description">“ASAL”  Мёд</h3>
            <p className="special__offer-old-price">3000</p>
            <h2 className="special__offer-price">17.<span>500</span></h2>
            <h2 className="special__offer-currency">UZS</h2>
            </div>
          </div>
        </div>
    
    <div className="special__offer-bottom-box">
      <h3 className="special__offer-bottom-title">Туалетная бумага “Нежность”</h3>
      <p className="special__offer-bottom-text">Вы с друзьями запланировали киномарафон? Не забудьте запастись чипсами на большую компанию.</p>
      <button className="special__offer-bottom-btn">Подробнее...</button>
        </div>
      </div>
    </div>

    <ul className="list__nav">
      <li className="list__nav-items"><span>1</span></li>
      <li className="list__nav-items">2</li>
      <li className="list__nav-items">3</li>
      <li className="list__nav-items">4</li>
      <li className="list__nav-items">5</li>
      <li className="list__nav-items">6</li>
    </ul>
</div>
    </section>
  )
}

export default Special